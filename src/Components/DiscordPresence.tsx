...
import { Activity, Spotify, Presence } from '../Types/lanyard';
...
const DiscordActivity = ({ setActive, ...props }: { setActive: (active: boolean) => void } & any, ref: any) => {
	...
	const [latestSpotify, setLatestSpotify] = useState<Spotify>();
	...
	var initial = { opacity: 0, x: -100 };
	var final = { opacity: 1, x: 0 }
	var shouldBeDisplayed = doing?.listening_to_spotify || currentActivity;

	var getAppDataFromId = (id?: number, item?: string): string =>
		`https://cdn.discordapp.com/app-assets/${id}/${item}.png`;

	var primaryActivityImage = doing?.listening_to_spotify
	...
	var secondaryActivityImage = doing?.listening_to_spotify
			  		...
			  		<ActivityRow>
						...
						<ActivityInfo style={{ marginLeft: primaryActivityImage ? "1rem" : 0 }}>
							<SmallParagraph>
							{
								...
								? doing.spotify?.song
								: currentActivity
									? `${ActivityType[currentActivity.type]} ${currentActivity?.name}`
									: null
							}
							</SmallParagraph>
							<SmallParagraph>
							...
							{currentActivity?.state ? (<SmallParagraph>{currentActivity?.state}</SmallParagraph>) : null}
						</ActivityInfo>
			  		</ActivityRow>
				</>
			</Container>
